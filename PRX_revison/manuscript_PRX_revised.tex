...
This picture also resolves an apparent paradox. In equilibrium statistical mechanics, increasing temperature weakens the preference for lower-energy states. In SGD, however, increasing the global noise strength (common effective-temperature) through larger learning rates or smaller batch sizes can lead to flatter final solutions, because stronger noise prolongs the pre-freezing regime, allowing the dynamics more time to follow the effective potential and discover flatter solutions. Like the Mpemba effect, a ``hotter'' SGD dynamics can ultimately freeze into a flatter, lower-effective-potential state. This physical picture offers a unified framework connecting stochasticity, loss landscape geometry, and generalization, and reveal an explicitly transient mechanism by which SGD preferentially selects flat minima.
...
\caption{Continuation training reveals transient valley hopping and freezing during SGD. (a) A PCA projection of the reference SGD trajectory (blue; $B=50$, $\eta=0.05$) and deterministic full-batch GD continuations branched from different checkpoint times $t_c$ (colored curves). The vertical coordinate indicates training loss. (b, c) Evolution of the test loss $\mathcal{L}_{\mathrm{test}}$ (b) and flatness $F$ (c) for the reference SGD trajectory and the GD-continued trajectories. {\color{red} In panel b\& c, the test loss first goes down then goes up while the Flatness continues to increase. The minimum of the test loss happens around $\sim 450$, which is similar to the freezing time. Are these two times related to each other? For each of the continuation experiments, maybe we should label their starting points in panel b? To show these starting points easier, maybe we should shrink the range of t in panel b\&c to 1500 or even 1000?  } (d) Jaccard similarity $Sim_J$ among solutions obtained from different continuation times $t_c$, with the red square marking the region where $t_c \geq t_{f}$. (e) Endpoint test loss and flatness (evaluated at $t=2000$) as functions of continuation time $t_c$. The vertical dashed line marks the same freezing time $t_f$ inferred from panel (d). {\color{red} For panel (e), it will be good to plot means and error bars (or thick lines) to show some statics over many (several) runs.} }
\label{fig:phenomenology}
\end{figure*}


\subsection{Stronger SGD noise delays the freezing transition}
...
{\color{red} The 3-subsections on the minimal model are quite long, they need to be shortened.... to better bring out the salient points without being too tedious. }

...
(c) Steady-state flat-valley probability $P_{\mathrm{flat}}^{\mathrm{ss,SGD}}(y,\Delta_S)$ from Eq.~\ref{eq:ss_SGD}. The red curve indicates the freezing boundary $y_{\mathrm{freeze}}(\Delta_S)$ estimated from Eq.~\ref{eq:y_freeze}. The black dashed line marks a fixed-$y$ slice at $y=5$.}}
\label{fig:analytical_results}
\end{figure*}


{\color{blue}Figure~\ref{fig:analytical_results}(b) provides the simulation counterpart of this reduced model. Individual trajectories initially cross the ridge at $x=0$ and later remain within one valley, while the ensemble probability $P_{\mathrm{flat}}(y)$ evolves with the slow coordinate $y$. These observations motivate treating valley selection as a combination of a fast quasi-steady-state bias at fixed $y$ and a separate freezing condition along the slowly evolving direction.}
...
\label{eq:tr_SGD}
\end{equation}
which, since $\gamma > 1$, shows that $P_{\mathrm{flat}}$ increases monotonically with noise strength within the same freezing-approximation regime. {\color{blue}In Fig.~\ref{fig:analytical_results}(c), this corresponds to moving along the red freezing boundary rather than along a fixed-$y$ slice.} Thus, while stronger noise reduces the static preference at fixed $y$, this effect is dominated by delayed freezing: the system explores longer and ultimately settles in the flatter valley with greater probability, resolving the apparent paradox.

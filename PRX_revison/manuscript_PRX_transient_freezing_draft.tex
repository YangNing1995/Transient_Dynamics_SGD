...
For fixed architecture and data, increasing the learning rate or decreasing the batch size increases the effective stochasticity of the update. We measure $t_f$ using the continuation-based basin-stability criterion: early checkpoints continue to endpoints belonging to different basin labels, whereas checkpoints after $t_f$ all relax to the same label under the same deterministic continuation readout. The full-batch condition provides a deterministic baseline for this readout, but its continuation-defined commitment time need not vanish because early points on a deterministic trajectory can still lie in regions whose continuation endpoint differs from the final one. Across trajectories that are continuation-stable and converged at the final checkpoint, the rescaled freezing coordinate $\eta t_f$ varies across both hyperparameters and repeats rather than forming a strictly monotone raw-parameter map, consistent with $t_f$ being a trajectory-level commitment time instead of a hyperparameter surrogate. Stronger stochasticity can keep trajectories mobile across competing valleys for a longer portion of training, but the central comparison below uses the measured freezing coordinate itself.
...
\caption{Freezing time organizes basin commitment and the geometry observed at commitment. (A) Median continuation-based freezing coordinate $\eta t_f$ across batch size and learning rate; gray cells indicate conditions without final-converged stable trajectories, and text marks stable fractions below 80\%. (C) Freezing-time flatness $F_f=-\log_{10}S_{\rm nw}(\theta_{t_f})$, measured from neuron-wise relative sharpness at the checkpoint corresponding to $t_f$, compared with $\eta/B$ (left) and with $\langle\eta t_f\rangle$ (right). This panel reports the local geometry present when the basin label becomes stable; it is not intended as a clock-independent estimate of final flatness. (D) Final test accuracy compared with the same two organizing variables. Point colors indicate learning rate; comparison panels omit the highest-learning-rate sweep and include continuation-stable trajectories that converge at the final checkpoint.}

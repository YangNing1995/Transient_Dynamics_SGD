...
(c) Steady-state flat-valley probability $P_{\mathrm{flat}}^{\mathrm{ss,SGD}}(y,\Delta_S)$ from Eq.~\ref{eq:ss_SGD}. The red curve indicates the freezing transition boundary $y_{\mathrm{freeze}}(\Delta_S)$ estimated from Eq.~\ref{eq:y_freeze}, which separate the exploration phase (below) from the frozen phase (above). The black dashed line marks a fixed-$y$ slice at $y=5$. {\color{red} If possible, it would be good to show $y_{freeze}$ (mean$\pm$ std) from direct simulation for a few values of $\Delta_S$ in panel C to compare with analytical result. A minor point: Can we make "Freezing transition" two lines (Freezing above transition) and more visible? Also, maybe we can make the red arrow pointing with an angle (not vertical)? }}

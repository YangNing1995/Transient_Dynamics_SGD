...
% across different learning rates and noise strengths (Cov ~ H^2)
% =========================================================================
...
learning_rate_list = logspace(-3, -1, 10); 
noise_strength_list = logspace(-3, -1, 10);
...
parfor idx = 1:total_combinations
    ...
    last_y_positions = zeros(num_repetition, 1); 
    freezing_times = nan(num_repetition, 1);     
    ...
    for n = 1:num_repetition
        ...
        % Run SGD Simulation (Modified for Cov ~ H^2)
        ...
        if ~isempty(sign_changes)
            freezing_times(n) = sign_changes(end) + 1;
        else
            freezing_times(n) = 1;
        end
    ...
    % Calculation of NESS probability (Using original theoretical formula)
...
% Updated Filename to indicate H^2 variance
file_name = strcat('result_PhsDgm_Ld', num2str(L_d), '_yd', num2str(y_d), '_x1', num2str(x1), '_x2', num2str(x2), '_H2.mat');
...
function [trajectory, loss_history] = stochastic_gradient_descent(Loss, grad_x, grad_y, Hessian, initial_pos, learning_rate, iterations, noise_strength)
    % MODIFIED for Cov = 2 * noise_strength * H^2
    ...
    for i = 1:iterations
        ...
        % Calculate Covariance Matrix: Cov = 2 * noise_strength * H^2
        % H^2 = H * H
        H2 = H * H;
        
        Cov = 2 * noise_strength * H2;
        
        % Ensure Cov is symmetric (H^2 is symmetric if H is symmetric)
        Cov = (Cov + Cov') / 2;
        
        % Add epsilon for numerical stability (ensure PSD)
        epsilon = 1e-8;
...
function P_flat = NESS_distribution(f1, f2, g1, Loss, learning_rate, noise_strength, y_val)
    % Note: This theoretical calculation assumes simple diffusion.
    % It may not accurately reflect the dynamics driven by Cov ~ H^2.

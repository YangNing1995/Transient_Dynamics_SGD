...
One of our key findings is that the freezing point $y_\mathrm{freeze}$ increases with the noise level $\Delta_S$. Intuitively, higher noise keeps the system ``hotter'' for longer, enabling continued inter-valley transitions and thus delaying the effective freezing point [Fig.~\ref{fig:analytical_results}(b)]. Quantitatively, $y_\mathrm{freeze}$ can be determined by requiring the escape rate from the sharper valley to become negligible compared to the evolution rate of the steady-state distribution, which leads to: 
\begin{equation}
...
\label{eq:y_freeze}
\end{equation}
where $\varepsilon$ is a small quantity and $\Phi$ is a constant that absorbs other model parameters (see Appendix~\ref{sec:appendix4} for detailed derivation). The dependence of $y_\mathrm{freeze}$ on $\Delta_S$ given in Eq.~\ref{eq:y_freeze} explains the increased mean freezing time $\eta \langle t_f \rangle$ with noise strength observed in both numerical simulations [Fig.~\ref{fig:toy_model}(c)] and empirical deep network experiments [Fig.~\ref{fig:freezing_control}(a)]. Plugging in the expression of $y_\mathrm{freeze}$ for $P_{\mathrm{flat}}^{\mathrm{tr,SGD}}$ (Eq.~\ref{eq:tr_SGD_original}), we have:
\begin{equation}
